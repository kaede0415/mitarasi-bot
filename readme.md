...
## 方法2
備考欄に記入してあるボットトークンをmain.pyの一番下にあるbot.runの中身をボットトークンに書き換えます。  
https://discord.com/api/oauth2/authorize?client_id=1023853147644506203&permissions=274877999104&scope=bot にアクセスし、ボットをDiscordサーバーに招待します。  
```pipenv shell``` をターミナルで実行し、プログラムを実行します。
